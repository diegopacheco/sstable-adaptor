...
public class ClusteringComparator implements Comparator<Clusterable>
{
    ...
    public Clustering makeSpecial(List<Object> values)
    {
        if (values.size() != size())
            throw new IllegalArgumentException(String.format("Invalid number of components, expecting %d but got %d", size(), values.size()));
